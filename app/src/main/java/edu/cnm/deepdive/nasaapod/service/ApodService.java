...
public interface ApodService { // any field we define in an 'interface' are constants.

  String DATE_FORMAT = "yyyy-MM-dd";
  DateFormat DATE_FORMATTER = new SimpleDateFormat(DATE_FORMAT);

  static ApodService getInstance() {
    return InstanceHolder.INSTANCE; // Lazy initialization of a singelton.
  }
  @GET("planetary/apod")
  Single<Apod> get(@Query("api_key") String api_key, @Query("date") String date);
